...
public final class ReportGenerator {
    ...
    private static void hoursAndChunks(StringBuilder b, PlayerStats s) {
        ...
        b.append("### 15.3 位置心跳（定时采样）\n\n");
        b.append("共 **").append(s.getHeartbeats()).append("** 次定时心跳（轨迹日志 trail/ 里还有一份人类可读版本）。\n\n");
        if (s.getLastHeartbeatAt() > 0L) {
            b.append("- 最后一次心跳：").append(Text.dateTime(s.getLastHeartbeatAt())).append(" — ")
                    .append(Text.esc(s.getLastHeartbeatInfo())).append("\n");
        }
        if (!s.getHeartbeatWorld().isEmpty()) {
            b.append("\n| 心跳所在世界 | 次数 |\n|---|---|\n");
            for (Map.Entry<String, Integer> e : s.getHeartbeatWorld().entrySet()) {
                b.append("| ").append(Text.esc(e.getKey())).append(" | ").append(e.getValue()).append(" |\n");
            }
            b.append("\n");
        }
        if (!s.getHeldItems().isEmpty()) {
            b.append("**心跳时常持的物品**\n\n");
            tableInt(b, "手持物品", "次数", s.getHeldItems());
        }
    }
