...
| 构建 | 目标平台 | 字节码 | 产物 |
| --- | --- | --- | --- |
| `1.12.2/` | Paper / Spigot 1.12.2 | Java 8 | `dist/PlayerInsight-1.12.2-4.jar` |
| `1.21.11/` | Paper 1.21.x | Java 21 | `dist/PlayerInsight-1.21.11-4.jar` |
...
1. **服务器里还放着旧 jar**：旧版 1.21.11 的文件名是 `PlayerInsight-1.21.11.jar`（v1），
   新的是 `PlayerInsight-1.21.11-4.jar`（v4）。名字不同不等于版本不同，看启动日志最准：

   ```text
   [PlayerInsight] Loading server plugin PlayerInsight v1.21.11-4
   [PlayerInsight] Enabling PlayerInsight v1.21.11-4
   ```
